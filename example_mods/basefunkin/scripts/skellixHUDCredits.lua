...
function onCreatePost()
    if creditsActive == true then
        ...
        if string.lower(difficultyName) == 'normal' then
            coolPeople = parseString(getTextFromFile('data/'..songPath..'/credits/credits.txt'), '%s')
        else
            coolPeople = parseString(getTextFromFile('data/'..songPath..'/credits/credits-'..string.lower(string.gsub(difficultyName, "%s+", "-"))..'.txt'), '%s')
        end
